...
public class FileManager2 {
	...
	public static void main(String args[]) {
		/*
		 * Path path=Paths.get("log/stdout.txt"); try { match(path,".*39=8.*");
		 * } catch (IOException e) {
		 * 
		 * e.printStackTrace(); }
		 */
	...
	public static void copy2(Path from, Path to) {
		...
		try (ReadableByteChannel rc = Files.newByteChannel(from, StandardOpenOption.READ);
				WritableByteChannel wc = Files.newByteChannel(to, StandardOpenOption.CREATE,
	...
	public static void genFile() {
		Path dir = Paths.get("log/test");
		Path f = Paths.get("log/test/t.txt");
		...
		try {
			...
			Files.write(f, "abc".getBytes(), StandardOpenOption.TRUNCATE_EXISTING);
		} catch (IOException e) {
